...
{
    if (TextureManager::GetInstance()->Has(name))
        return S_OK; // 既にロード済み
    ...
    //------------------------------------------------------------------
    {
        ...
        if (SUCCEEDED(hr)) {
            ...
            DX12::CommandContext* cmdCtx = dx->GetCommandContext();
            DX12::DescriptorAllocator* allocator = dx->GetPersistentDescAlloc();
...
Texture* TextureLoader::CreateTextureFromMemory(
    ...
    UINT width, UINT height, DXGI_FORMAT format, UINT bytesPerPixel)
{
    ...
    Texture* ret = tex.get();
    TextureManager::GetInstance()->Emplace(name, tex.get());
