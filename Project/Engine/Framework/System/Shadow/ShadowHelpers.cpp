...
using namespace Tsumi::Framework::ShadowDetail;

// =========================================================
// ShadowMath
// =========================================================

Tsumi::Math::Vec3f ShadowMath::NormalizeSafe(const Math::Vec3f& v, const Math::Vec3f& fallback)
{
...
Tsumi::Math::Vec3f ShadowMath::Average8(const Math::Vec3f p[8])
{
	Math::Vec3f s{ 0,0,0 };
...
)
{
	...
	for (uint32_t i = 0; i < config.cascadeCount; ++i)
	{
		...
		BuildCascade(i, camera, lightDir, config.shadowMapSize, outCtx);
	}
}
...
)
{
	...
	float dist = sphereRadius + 1000.0f; // Sufficient padding
	...
	float worldUnitsPerTexel = (sphereRadius * 2.0f) / shadowMapSize;
	...
	// Recalculate View with Snapped Center
	...
	float farZ_LS = dist + r + 500.0f; // buffer
	float nearZ_LS = dist - r - 500.0f; // buffer
