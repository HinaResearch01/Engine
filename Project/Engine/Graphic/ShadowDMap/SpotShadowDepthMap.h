...
class SpotShadowDepthMap {

public:
	/// <summary>
	/// コンストラクタ
	/// </summary>
	SpotShadowDepthMap() = default;
	...
	/// 初期化処理
	/// </summary>
	void Init(uint32_t size, uint32_t count);

	/// <summary>
	/// リサイズ
	/// </summary>
	void Resize(uint32_t size, uint32_t count);
...
#pragma region Accessor
	...
	// DSV（indexごと）
	D3D12_CPU_DESCRIPTOR_HANDLE GetDSV(uint32_t index) const;
	const D3D12_CPU_DESCRIPTOR_HANDLE* GetDSVPtr(uint32_t index) const;
...
private:
	...
	std::vector<D3D12_CPU_DESCRIPTOR_HANDLE> dsvCpu_; // [count]
